...
import { Modal } from "bootstrap"; // Asegúrate de que Bootstrap esté importado correctamente
...
export const Navbar = () => {
  ...
  // Traslada el foco al cerrar el modal para evitar problemas de accesibilidad
  useEffect(() => {
  ...
  // Función para alternar el colapso, comprobando el estado actual
  const toggleNavbar = () => {
    if (collapseRef.current && window.bootstrap) {
      // Si el colapso está desplegado, lo escondemos; de lo contrario, lo mostramos
                  ...
                  {Object.entries(gruposVehiculos).map(([categoria, lista]) =>
                    lista.length > 0 && (
                      <React.Fragment key={categoria}>
                        <li>
                          <h6 className="dropdown-header">{categoria}</h6>
                        </li>
                        {lista.map((vehiculo) => (
                          <li key={vehiculo.id}>
                            <Link
